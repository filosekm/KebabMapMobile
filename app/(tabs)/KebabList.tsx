  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(
        "https://kebabapipanel-tg6o.onrender.com/api/kebabs/legnica",
      );
      if (!response.ok) {
  ...
  const filteredData = data
    .filter((item) => {
      const matchesStatus = filters.status
        ...
        : true;
      const matchesCraftRating =
        filters.craftRating !== null
          ? item.craftRating === filters.craftRating
          : true;
      const matchesInChain =
        filters.inChain !== null ? item.inChain === filters.inChain : true;
    ...
    <View style={styles.container}>
      <View style={styles.headerRow}>
        ...
        <ThemedText style={styles.title} type="title">
          Lista Legnickich Kebabów
        </ThemedText>
      </View>
      <Text style={styles.totalCount}>
        Łączna liczba kebabów: {totalKebabs}
      </Text>
              ...
              {["Wszystkie", "Craft", "Non-Craft"].map((label, index) => {
                ...
                return (
                  <TouchableOpacity
                    key={label}
                    onPress={() =>
                      setFilters({ ...filters, craftRating: value })
                    }
                  >
                    <Text
                      style={[
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={styles.item}
                  onPress={() =>
                    router.push(`/screens/KebabDetails?markerId=${item.id}`)
                  }
                >
